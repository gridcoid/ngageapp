<template>
  <div class="containers p-6 w-full bg-gradient">
    <!-- Header -->
...
export default {
  ...
  methods: {
    ...
    toCreate() {
      this.$router.push({ path: '/direct/audience/create' })
    },
    ...
    deleteAudience(data) {
      this.$confirm(
        ...
        .then(() => {
          ...
          this.$store
            .dispatch('audience/delete', { uuid: data.uuid })
            .then((res) => {
              if (res.status === 204) {
                this.getData()
    ...
    viewDetail(item) {
      this.$router.push({ path: '/direct/audience/detail/' + item.uuid })
    },
