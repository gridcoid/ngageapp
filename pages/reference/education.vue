...
export default {
  ...
  head() {
    return {
      title: 'Educations - ' + this.$config.appName,
    }
  },
  data() {
    return {
  ...
  computed: {
    ...mapState({
      ...
      totalList: (state) => state.education.totalList,
      totalPages: (state) => state.education.totalPages,
    }),
  },
  mounted() {
    this.getData()
  ...
  methods: {
    getData() {
      this.isLoading = true
      this.$store.dispatch('education/list').finally(() => {
