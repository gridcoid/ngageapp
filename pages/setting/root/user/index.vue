<template>
  <div class="kg-containers p-6 w-full">
    ...
    <div class="flex items-center filter-content justify-between">
      <div class="desc-page">
        Manage system users, control activation, and assign organizations.
      </div>
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.$store
        .dispatch('rootUser/rootList', payload)
        .finally(() => (this.isLoading = false))
    },

    toCreate() {
      this.$router.push('/setting/root/user/create')
    },
    ...
    deleteUser(data) {
      this.$confirm(`Delete user "${data.username}"?`, 'Confirmation', {
        ...
        .then(() => {
          ...
          this.$store
            .dispatch('rootUser/rootDelete', { uuid: data.uuid })
            .then((res) => {
              if (res.status === 204) {
                this.getData()
