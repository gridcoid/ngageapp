<template>
  <div class="option-card">
    <div v-for="(item, index) in dataOption" v-if="!isLoading" :key="index" class=" cursor-pointer no-select list-card flex items-center justify-between">
      <div class="flex items-center justify-start" style="width:70%" @click="changeStatus(item, index)">
        <div class="circle flex items-center justify-center" :style="item.status ? 'border: 1px solid #1B63D4;' : ''">
      ...
      <img :id="'audioControl' + index" src="https://dev-space.unimind.id/icon/play_v1.png" style="height:15px;margin-right:5px;" @click="togglePlay(index)">
    </div>
  </div>
</template>
<script>
