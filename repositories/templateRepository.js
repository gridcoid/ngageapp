...
export default ($axios) => ({
  ...
  getDetail(payload) {
    const orgId = window.$nuxt.$store.state.user.orgId
    return $axios.get(
      `${resource}/${payload.id}?orgId=${orgId}&resolutionId=${payload.resolution}`
    )
  },
  ...
  getPreviewWebsites(payload) {
    const orgId = window.$nuxt.$store.state.user.orgId
    return $axios.get(
      `${resource}/preview-website/${payload.id}?orgId=${orgId}`
    )
  },
  ...
  getScrapeImage(payload) {
    return $axios.get(
      `scrape/kg-rhythm/image?url=${payload.url}&multiple=${payload.multiple}`
    )
  },
  ...
  getScrapeWeather(payload) {
    return $axios.get(
      `scrape/weather?date=${payload.date}&island=${payload.island}`
    )
  },
})
